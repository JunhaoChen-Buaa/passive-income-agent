...
核心流程是：先让 Agent 读取用户画像、现金池、市场温度、回测数据和投资记录，再生成个人策略、行动指南和纪律复盘。

## AI Native 设计

这个项目不是把 AI 放在最后写一段解释，而是把 AI 作为“长期投资纪律副驾驶”：

- AI 工作台：展示 Agent 已读取哪些上下文、调用哪些工具、发现哪些数据缺口。
- AI 个人策略：通过用户画像生成长期指数配置草案，并自动调用回测工具验证历史体验。
- 上下文问一问：对话会读取当前策略、市场温度、本月计划和投资记录，而不是只回答通用 FAQ。
- 纪律复盘：根据投资记录复盘是否执行计划，帮助用户发现追涨、停投、重复保存等行为偏离。
- 规则边界：买入金额、现金安全垫、再平衡和卖出提醒仍由规则引擎生成，AI 不能覆盖金额。
...
- AI 工作台：可见化 Agent 的工具调用、数据缺口、下一步动作和建议追问。
- 上下文问一问：读取当前用户策略、市场状态和投资记录后再回答。
